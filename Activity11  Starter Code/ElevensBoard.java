...
public class ElevensBoard extends Board {
    ...
    public boolean isLegal(List<Integer> selectedCards) {
        ...
        //List<Integer> sumEleven = findPairSum11(selectedCards);
        if (findPairSum11(cIndexes).size() > 1) {
            return true;      
        } else if (findJQK(cIndexes).size() > 2) {
            return true;
        } else {
            return false;
        }
    ...
    private List<Integer> findPairSum11(List<Integer> selectedCards) {
        ...
        List<Integer> remove11 = new ArrayList<Integer>();
        for (int sk1 = 0; sk1 < selectedCards.size(); sk1++) {
            int k1 = selectedCards.get(sk1).intValue();
            for (int sk2 = sk1 + 1; sk2 < selectedCards.size(); sk2++) {
                int k2 = selectedCards.get(sk2).intValue();
                if ((cardAt(k1).pointValue() + cardAt(k2).pointValue() == 11) && remove11.size() < 1){
                    remove11.add(k1);
                    remove11.add(k2);
                    return(remove11);
                }
    ...
    private List<Integer> findJQK(List<Integer> selectedCards) {
        ...
        List<Integer> removeJQK= new ArrayList<Integer>();
    ...
    private boolean playPairSum11IfPossible() {
        /* *** TO BE IMPLEMENTED IN ACTIVITY 11 *** */
        List<Integer> cIndexes = cardIndexes();
        List<Integer> u = findPairSum11(cIndexes);
        if (u.size() > 1) {
            for (int i = 0; i<2; i++){
                replaceSelectedCards(u);
            }
            return true;
        } else {
            return false;
    ...
    private boolean playJQKIfPossible() {
        /* *** TO BE IMPLEMENTED IN ACTIVITY 11 *** */
        List<Integer> cIndexes = cardIndexes();
        List<Integer> u = findJQK(cIndexes);
        if (u.size() > 1) {
            for (int i = 0; i<3; i++){
                replaceSelectedCards(u);
            }
